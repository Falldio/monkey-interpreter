...
func TestDefine(t *testing.T) {
	...
	a := global.Define("a")
	if a != expected["a"] {
		t.Fatalf("a is not expected symbol. got=%+v", a)
	}

	b := global.Define("b")
	if b != expected["b"] {
		t.Fatalf("b is not expected symbol. got=%+v", b)
	}
	...
	if c != expected["c"] {
		t.Fatalf("c is not expected symbol. got=%+v", c)
	}

	d := firstLocal.Define("d")
	if d != expected["d"] {
		t.Fatalf("d is not expected symbol. got=%+v", d)
	}
	...
	if e != expected["e"] {
		t.Fatalf("e is not expected symbol. got=%+v", e)
	}

	f := secondLocal.Define("f")
	if f != expected["f"] {
		t.Fatalf("f is not expected symbol. got=%+v", f)
	}
}
...
func TestResolveFree(t *testing.T) {
	...
	for _, tt := range tests {
		...
		for i, sym := range tt.expectedFreeSymbols {
			...
			if result != sym {
				t.Errorf("expected %s at index %d, got %+v, want %+v", sym.Name, i, result, sym)
			}
		}
	}
}
...
func TestResolveUnresolvableFree(t *testing.T) {
	...
	for _, name := range expectedUnresolvable {
		...
		if ok {
			t.Errorf("name %s resolved incorrectly", name)
		}
	}
}
...
func TestShadowingFunctionName(t *testing.T) {
	...
	result, ok := global.Resolve(expected.Name)
	if !ok {
		t.Fatalf("name %s not resolvable", expected.Name)
	}
